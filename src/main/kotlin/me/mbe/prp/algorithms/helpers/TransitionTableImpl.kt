...
class TransitionTableImpl<K, L>(
    ...
    override fun addTransitionInternal(from: K, to: L, weight: Double, duration: Duration) {
    ...
    override fun getNextWithProbAllInternal(from: K): List<Triple<L, Duration, Double>> {
        val e = map[from]?.entries ?: return emptyList()
        val s = e.sumOf { it.value.first }

        return if (storeDuration)
            e.map { Triple(it.key, reducer(it.value.second, it.value.first), it.value.first / s) }
        else
